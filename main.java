	...
	public static void main(String[] args) {
	    ...
	    while (sc.hasNextLine()) {										// reading line by line
	            ...
	            if (command.equals("add_item")) {							
	            	...
	            	String[] splittedCommandArguments = arguments.split(";", -2);  				// for getting arguments word by word, split the arguments from ";"
		           ...
		           //////////////////     	   		  COMMAND					\\\\\\\\\\\\\\\\\\\\
	            	...
	            	if(currentEmployerCount < MAX_EMPLOYER) {											// if total count of the employers are less than MAX_EMPLOYER number
		            	...
		            	newEmployer.setSalary(Double.parseDouble(splittedCommandArguments[1]));			// second word is new employer's salary
		           ...
		           //////////////////     	   		 COMMAND					\\\\\\\\\\\\\\\\\\\\  
	            	...
	            	if(currentWaiterCount < MAX_WAITER) {			// if total count of the waiters are less than MAX_WAITER number
		            	...
		            	newWaiter.setSalary(Double.parseDouble(splittedCommandArguments[1]));			// second word is waiter's salary
		            	newWaiter.setOrderCount(0);													// third word is waiter's total order count
		            	newWaiter.setTableCount(0);
		            	waiters [currentWaiterCount] = newWaiter;newWaiter.orderCountForSalary = 0;									// put the created object into the waiters array		            	
	    ...
	    while (sc2.hasNextLine()) {											// reading line by line
	           ...
	           //////////////////     	   		   COMMAND					\\\\\\\\\\\\\\\\\\\\
	            ...
	            if (command.equals("create_table")) {											// if command is create_table
	            	...
	            	for (int i = 0; i<currentEmployerCount; i ++) {								// searching in employers array to find the employer that is given 
	            	...
	            	if(currenTableCount < MAX_TABLES && employerFound == 1 && employerNumber > -1 && employers[employerNumber].getTableCount() < ALLOWED_MAX_TABLES ) {   // if all rules are followed to create a table
	            		
	            		Tables newTable = new Tables();													// create an object from Tables class
		            	
		            	newTable.setID(currenTableCount);												// set the object's ID
		            	newTable.setInService(0);														// set the object's service information as zero. Because the table is currently opened
		            	newTable.setCapacity(Integer.parseInt(splittedCommandArguments[1]));			// set the object's capacity as second word of the splitted arguments
		            	newTable.setCreator(splittedCommandArguments[0]); 								// set the object's creator as first word of the splitted arguments
		            	newTable.setTotalOrderCount(0);													// set the object's total order count as zero. Because the table is currently opened
	            	...
	            	else if(employers[employerNumber].getTableCount() >= ALLOWED_MAX_TABLES) {			//  if the number of tables creator owns is greater than ALLOWED_MAX_TABLES
		           ...
		           //////////////////     	   			 COMMAND					\\\\\\\\\\\\\\\\\\\\
	            	 ...
	            	 arguments = forCommands.next();
	            	 String[] splittedCommandArguments = arguments.split(";", -2);			// for getting arguments word by word, split the arguments from ";"
	            	 int tableID = -1;														// checks the searching table is found or not if its found then holds the given tables ID 
	            	 int waiterNumber = -1;													// checks the searching waiter is found or not if its found then holds the given waiter's index in the waiters array 
	            	 
	            	 for(int i = 0; i<currenTableCount; i++) {        						// finding the tableID 
	            		 
	            		 if(tables[i].getInService() == 0 && tables[i].getCapacity() >= Integer.parseInt(splittedCommandArguments[1]) ) {		// if we find the table
	            			 
	            			 tableID = i;  break;			 
	            		 }
	            	 }
	            	 
	            	 for (int i = 0; i<currentWaiterCount; i ++) {								// searching in waiters array to find the waiter that is given 
	            	 ...
	            	 if(tableID == -1) {																		// if the table is not in tables array
	            	 ...
	            	 else if(waiters[waiterNumber].getOrderCount() >= MAX_TABLE_SERVICES) {		// if the waiter's order count is greater than MAX_ORDERS
	            		 ...
	            		 String[] splittedItemArguments = splittedCommandArguments[2].split(":", -2);      		// splittedItemArguments elements are items in new order
	            		 ...
	            		 tables[tableID].setInService(1);														// set table's service information as 1. This represents the table is busy
	            		 tables[tableID].createOrderList();														// call createOrderList from Tables class. This method is explained in Tables class 
		            	 ...
		            	 for (String anItem : splittedItemArguments) {											// loop for the words in command's arguments
		            		 ...
		            		 String[] splittedItems = anItem.split("-", -2);									// splittedItems' first element is item, second element is amount of the item
		            			 ...
		            			 if (splittedItems[0].equals(items[i].getItemName()) ) {						// if given item is in items array
		            				 ...
		            				 for (int j = 0; j< Integer.parseInt(splittedItems[1]); j++) {				// loop in item's amount 
		            					 ...
		            					 if( 1 <= items[i].getStockCount() ) {	            					// if the item has a sufficient number in stock	 
		            						 if (tables[tableID].orders[0].getTotalItemCount() == tables[tableID].orders[0].MAX_ITEMS) System.out.println("Not allowed to exceed max no. of max item in a single order!");           	 
		            						 else{tables[tableID].addfirstOrder(items[i].getItemName(),items[i].getItemCost(),items[i].getStockCount());		// call addFirstOrder method in Tables class
		            							 System.out.println("Item "+ items[i].getItemName() +" added into order");
		            							 items[i].decrementStockCount();									// decrement the stock count of the ordered item in items array
		            						 }
		            					 }
		            					 else if(items[i].getStockCount() == 0) {								// if the item's amount is zero (in items array)
		            		 ...
		            		 if(itemFound == 0) { System.out.println("Unknown item " + splittedItems[0]);}	         // if given item is not in items array   		 
		           ...
		           //////////////////     	   		   COMMAND					\\\\\\\\\\\\\\\\\\\\
	            	 ...
	            	 arguments = forCommands.next();
	            	 String[] splittedCommandArguments = arguments.split(";", -2);		// for getting arguments word by word, split the arguments from ";"
	            	 int waiterNumber = -1;												// checks the searching table is found or not if its found then holds the given tables ID 
	            	 int tableID = -1;													// checks the searching waiter is found or not if its found then holds the given waiter's index in the waiters array 
	            	
	            	 
	            	 for (int i = 0; i<currentWaiterCount; i ++) {								// searching in waiters object array to find the waiter that is given 
	            	 ...
	            	 if(waiterNumber == -1) {																// if the waiter is not in waiters array
	            	 ...
	            	 else if(tables[tableID].getWaiter().equals(splittedCommandArguments[0]) == false || tables[tableID].getInService() == 0) {  // if given waiter is not the table's waiter or the table is not busy
	            		 
	            		 System.out.println("This table is either not in service now or "+ splittedCommandArguments[0] +" cannot be assigned this table!");
	            	 }
	            	 else if(tables[tableID].getTotalOrderCount() >= tables[tableID].getMAX_ORDERS()) {				// if the table's order count is greater than or equal MAX_ORDER
	            		 ...
	            		 String[] splittedItemArguments = splittedCommandArguments[2].split(":", -2);		// splittedItemArguments elements are items in add order
		            		 ...
		            		 for (int i = 0; i < currentItemCount; i++) {									// loop in items array
		            			 ...
		            			 if (splittedItems[0].equals(items[i].getItemName()) ) {					// if given item is in items array
		            				 ...
		            				 for (int j = 0; j< Integer.parseInt(splittedItems[1]); j++) {							// loop in item's amount 
		            					 ...
		            					 if( 1 <= items[i].getStockCount() ) {	            								// if the item has a sufficient number in stock	  		 
		            						 if (tables[tableID].orders[tables[tableID].totalOrderCount].getTotalItemCount() == tables[tableID].orders[tables[tableID].totalOrderCount].MAX_ITEMS) System.out.println("Not allowed to exceed max no. of max item in a single order!");           	 
				            				else{tables[tableID].addfirstOrder(items[i].getItemName(),items[i].getItemCost(),items[i].getStockCount());		// call addFirstOrder method in Tables class
					            					System.out.println("Item "+ items[i].getItemName() +" added into order");
					            					items[i].decrementStockCount();									// decrement the stock count of the ordered item in items array
				            				}}
		           ...
		           //////////////////     	   			 COMMAND					\\\\\\\\\\\\\\\\\\\\
	            	 ...
	            	 for(int i = 0; i<currenTableCount; i++) {
	            		 ...
	            		 if(tables[i].getInService() == 1 &&  Integer.parseInt(splittedCommandArguments[1]) == tables[i].getID()) {		 // if we find the table
	            	 ...
	            	 if (waiterNumber == -1) {  																						// if given waiter is not in waiters array
	            	 ...
	            	 else if(tableID == -1 || tables[tableID].getInService() == 0 || tables[tableID].getWaiter().equals(splittedCommandArguments[0]) == false) {		// if the table is not found in tables array or given table is not in service or given waiter is not the table's waiter
	            		 
	            		 System.out.println("This table is either not in service now or "+ waiters[waiterNumber].getName() +" cannot be assigned this table!");
	            		 
	            	 }
	            	 else {											// if all rules are followed to adding an order
	            		 
	            		 String array[] = new String[1000];			// temporary array that is holds the unique items and the repeating count in the orders. 
	            		 int foundIndex = -1;						// if coming item is in array then the variable holds the item's index into the array[] else the variable holds -1
	            		 ...
	            		 for (int i = 0 ; i < tables[tableID].totalOrderCount; i++ ) {							// loop for a table's orders
	            			 for(int j = 0; j < tables[tableID].orders[i].getTotalItemCount(); j++ ) {			// loop for the table's orders' items
	            				 foundIndex = -1;																// for each item the variable is initially -1
	            				 for(int k= 0; k< array.length; k = k + 2) {									// loop for the array's length/2. Because the array holds 2 value for each item. First index for item's name, second is for item's repeat count
	            					 if (tables[tableID].orders[i].items[j].itemName.equals(array[k])) {		// if the item is in array[]
	            						 foundIndex = k;
	            				 ...
	            				 if(foundIndex == -1) {													// if the item is not in array[]
	            					 array[counter] = tables[tableID].orders[i].items[j].itemName;		// put the item's name into the array[]
	            		 ...
	            		 for (int l = 0; array[l] != null; l = l + 2) { 				// for printing the array[] elements
	            			 ...
	            			 for (int i = 0; i < currentItemCount; i++) {     			// for finding the array[] elements in items[]
		            			 ...
		            			 if( array[l].equals(items[i].getItemName()) ) {		// find the array[] elements in items[]
		            				 ...
		            				 System.out.println( array[l] + ":\t" +  formatter.format(items[i].getItemCost()) + " (x "+array[l+1] +") " + formatter.format(items[i].getItemCost()*Double.parseDouble(array[l+1]))+ " $");
		            			 }
		     		  		 }
	            			 
	            		 } 
	            		 System.out.println("Total:\t"+ formatter.format(total)+" $");
	            		 tables[tableID].setInService(0);waiters[waiterNumber].orderCount --;tables[tableID].totalOrderCount = 0;   		 
	            	 }          	  
	            }
		           ...
		           //////////////////     	   			COMMAND					\\\\\\\\\\\\\\\\\\\\
	            	...
	            	for(int i = 0; i<currenTableCount; i++) {										// loop for tables array
	            		 ...
	            		 if (tables[i].getInService() == 0 ) {										// if the table is not in service
	            			 ...
	            			 for(int j= 0; tables[i].totalOrderCount > j; j++) {						// loop for tables array's elements's order elements
		           ...
		           //////////////////     	   			 COMMAND					\\\\\\\\\\\\\\\\\\\\
	            	...
	            	for(int i = 0; i<currentEmployerCount; i++) {									// loop for employers array
	            		...
	            		for(int j = 0; j<currenTableCount; j++) {									// loop for tables array
	            			...
	            			if(tables[j].getCreator().equals(employers[i].getName())) {				// if given employer is a table's creator
	            				
	            				tableCount ++;														// increment the employer's tableCount
	            			}	
	            		}        		
	            		System.out.println("Salary for "+ employers[i].getName() +": " + (tableCount*employers[i].getSalary()*0.1+ employers[i].getSalary() ));
		           ...
		           //////////////////     	   			COMMAND						\\\\\\\\\\\\\\\\\\\\
	            	...
	            	for(int i = 0; i<currentWaiterCount; i++) {										// loop for waiters array
	            		...
	            		System.out.println("Salary for "+ waiters[i].getName() +": " + (waiters[i].orderCountForSalary*waiters[i].getSalary()*0.05+ waiters[i].getSalary() ));
